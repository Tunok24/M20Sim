...
cat <<-labels | tee -a ./tmp_gnuplot.dbug > $GNUPLOT
	...
	set label "Septum_D" at 21716,110 center font ",\$fontsize"
	set label "XSLIT1D" at 22321.6,170 center font ",\$fontsize"
...
param B2 92.417056 70 100 2 limited
param Q8T 0.029556707 -1 1 0.005 limited
param Q8B 42.720258 30 50 1 limited
param Q9 -23.36234 -30 -10 1 limited
param Q10DB -0.012752652 -1 1 0.005 limited
param Q10DT 1.2561858 -100 100 1 limited
param Q10CB -0.0087409701 -1 1 0.005 fixed
param Q10CT 2.7418251 -10 10 1 fixed
param Q11DL -0.61812826 -1 1 0.005 limited
param Q11DR -75.548041 -85 -70 1 limited
param Q11CL 0.051537266 -0.1 0.1 0.01 fixed
param Q11CR -77.658069 -85 -70 3 fixed
param Q12D 110.99466 90 130 2 limited
param Q12C 104.79 -200 200 3 fixed
param SM1D 595.77564 550 625 5 limited
param SM1C 537.82754 450 775 20 fixed
param K1 9493.5595 7000 9500 20 limited
...
echo >&2 Executing: G4BL with 52 cores, Nfomlimit=1 nEvents=1000000 "\$@"
g4blmpi 52 "$G4BL" BL=1 nEvents=1000000 gminuit=1 PROFILE_D=$PROFILE PROFILE_GNUPLOT=$PROFILE_GNUPLOT   "\$@" >$OUT 2>&1

echo >$GNUPLOT 'plot [0:26300][-200:200] "$APERTURE" with lines notitle, "$PROFILE" using 1:(1.414*\$4) with lines title "sigmaX", "$PROFILE" using 1:(-1.414*\$6) with lines title "sigmaY", "$PROFILE" using 1:(\$2/5000) with lines title "Nmu"'

# chisq = sum of sigmaX^2 and sigmaY^2 multiplied with a normalized muon population at focus, Z=26278
