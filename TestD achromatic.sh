...
PROFILE='m20bl_profileD.txt'
PROFILE_dP='m20bl_profileDdP.txt'
...
then
	...
	cat <<-! >$GMINUIT
param P 28.647962 25 30 1 limited
...
param Q1 -32.459055 -150 150 3 limited
param Q2 71.392977 -150 150 3 limited
param B1 92.475974 50 120 3 limited
param Q3 78.473732 -150 150 3 limited
param Q4 -77.534067 -200 200 3 limited
param Q5 69.197331 -200 200 3 limited
param Q6 -91.640648 -200 200 3 limited
param Q7 95.232148 -200 200 3 limited
param B2 92.473556 70 100 5 limited
param Q8T 0.20620102 -1 1 0.005 limited
param Q8B 41.810258 -100 100 1 limited
param Q9 -21.564409 -100 100 1 limited
param Q10DB -0.01663766 -1 1 0.005 limited
param Q10DT 0.68211732 -100 100 1 limited
param Q10CB -0.0087409701 -1 1 0.005 fixed
param Q10CT 2.7418251 -200 200 2 fixed
param Q11DL -0.61942555 -1 1 0.005 limited
param Q11DR -75.573346 -200 200 1 limited
param Q11CL 0.051537266 -0.1 0.1 0.01 fixed
param Q11CR -77.658069 -200 200 3 fixed
param Q12D 111.17829 -200 200 2 limited
param Q12C 104.79 -200 200 3 fixed
param SM1D 591.77515 450 775 5 limited
param SM1C 537.82754 450 775 20 fixed
param K1 8408.1896 7000 9500 20 limited
...
if	[ "$HOSTNAME" = "musimr" ]; then nCores="48"; elif [ "$HOSTNAME" = "musim0" ]; then nCores="10"; else nCores="6"; fi 
echo >&2 Executing: G4BL with \$nCores cores, Nfomlimit=1 nEvents=100000 "\$@"
g4blmpi \$nCores "$G4BL" BL=1 dP=0 nEvents=100000 gminuit=1 PROFILE_D=$PROFILE_dP PROFILE_GNUPLOT=$PROFILE_GNUPLOT	 "\$@" >$OUT 2>&1
g4blmpi \$nCores "$G4BL" BL=1 dP=0.7 nEvents=100000 gminuit=1 PROFILE_D=$PROFILE PROFILE_GNUPLOT=$PROFILE_GNUPLOT	 "\$@" >$OUT 2>&1

echo >$GNUPLOT 'plot [0:26500][-200:200] "$APERTURE" with lines notitle, "$PROFILE" using 1:(1.414*\$4) with lines linecolor 4 title "sigmaX dP 0.6", "$PROFILE" using 1:(-1.414*\$6) with lines linecolor 4 title "sigmaY dP 0.6", "$PROFILE_dP" using 1:(1.414*\$4) with lines linecolor 10 title "sigmaX dP 0", "$PROFILE_dP" using 1:(-1.414*\$6) with lines linecolor 10 title "sigmaY dP 0", "$PROFILE" using 1:(\$2/500) with lines title "Nmu"'

#"$PROFILE" using 1:(\$2/500) with lines title "Nmu" \\


# chisq = sum of sigmaX^2 and sigmaY^2 multiplied with a normalized muon population at the end at Z=26278
FOM=\`awk <$PROFILE '/^26278/{print (\$3*\$3+\$4*\$4+\$5*\$5+\$6*\$6)*(100000/(\$2+1))^2}'\`
if ["\$FOM" == ""]; 
 then FOM=9E19
fi
echo >&2 "Chisq=\$FOM   \$@"
